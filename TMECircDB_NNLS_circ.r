...
# Run this script after you have finished CIBERSORT's “Impute Cell Fractions” module and running TMECircDB_CIRCexplorer_sum.r
...
# Expression matrix, every circRNA per row, every sample per column
# aggregate_circRNA_norm_counts.txt is an example of our data generated by TMECircDB_CIRCexplorer_sum.r, and is available at https://github.com/ChrisMaherLab/TMECircDB/sample_files
expression_all <- read.table("aggregate_circRNA_norm_counts.txt",header=T,sep='\t',stringsAsFactors = F);
rownames(expression_all) <- expression_all$uniq_identifier;
expression_all <- expression_all[4:33];
...
cibersort_all <- cibersort_all[ colnames(expression_all), ];
cibersort_all <- cibersort_all[, order(colnames(cibersort_all)) ];

output_df <- data.frame(rownames(expression_all));
output_df[,paste0(colnames(cibersort_all),"_coeff")] <- NA;
output_df[,"PVal"] <- NA;
output_df[,"RSquared"] <- NA;
colnames(output_df)[1] <- "circRNA";


nnls_cibersort <- function(circRNA)
{
  print(paste0("Running NNLS for circRNA: ",circRNA));
  
  A<- as.matrix(cibersort_all);
  B<- as.numeric(expression_all[rownames(expression_all)==circRNA,]);
...
output_df[,2:9] <-apply(sapply(output_df$circRNA,nnls_cibersort),1,t);
...
write.table(output_df,"circRNA_nnls_result.txt",quote=F,sep="\t",col.names=T,row.names=F)

## Benchmark NNLS model results per sample
# Read the output file generated in the step above and make into a matrix
pred_matrix <- read.table("circRNA_nnls_result.txt",header=T,sep='\t',stringsAsFactors = F);
rownames(pred_matrix) <- pred_matrix$circRNA;
...
pred_matrix <- read.table("circRNA_nnls_result.txt",header=T,sep='\t',stringsAsFactors = F);
rownames(matrix_multiplication) <- pred_matrix$circRNA;
...
write.table(cor_by_sample,"circRNA_nnls_corr_by_sample.txt",quote=F,sep="\t",col.names=T,row.names=F)
